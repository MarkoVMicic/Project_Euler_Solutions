...
main(Num_digits) ->
    ...
    [Factor1, Factor2] = find_largest_palindrome_prod_2_ints(Max_number, 
                                                             Num_digits),
...
find_max_factor(Num_digits) ->
    find_max_factor(9, 1, Num_digits).

find_max_factor(Current_number, Current_digits, Required_Digits) 
    when Current_digits =:= Required_Digits ->
    Current_number;
find_max_factor(Current_number, Current_digits, Required_Digits) ->
    ...
    find_max_factor(New_number, Current_digits + 1, Required_Digits).

find_largest_palindrome_prod_2_ints(Number, Num_digits) ->
    % Finds the largest palindrome =< number that is the product of 2 integers.
    Max_factor = find_max_factor(Num_digits),
    find_largest_palindrome_prod_2_ints(Max_factor, Number, Max_factor).
find_largest_palindrome_prod_2_ints(Current_factor, Number, Max_factor) ->    
    % • If Number is not a palindrome, decrease by one. 
    % • If Number is a Palindrome, Then start keeping track of a quotient, 
    %   call find_largest_palindrome_prod_2_ints/4
    case (is_palindrome(Number)) of
        true -> 
            find_largest_palindrome_prod_2_ints(Current_factor, Number, Max_factor, Number div Current_factor);
        false -> 
            find_largest_palindrome_prod_2_ints(Current_factor,
                                                Number - 1,
                                                Max_factor)
    end.
...
find_largest_palindrome_prod_2_ints(Current_factor, Palindrome, Max_factor, _)
    when Palindrome rem Current_factor == 0 
    andalso Palindrome div Current_factor < Max_factor ->
...
% find_largest_palindrome_prod_2_ints/3
find_largest_palindrome_prod_2_ints(Current_factor, Palindrome, Max_factor, _)
    when Palindrome div Current_factor > Max_factor ->
    find_largest_palindrome_prod_2_ints(Max_factor, Palindrome-1, Max_factor);
...
find_largest_palindrome_prod_2_ints(Current_factor, Palindrome, Max_factor, _)
    when Palindrome rem Current_factor =/= 0 ->
    find_largest_palindrome_prod_2_ints(Current_factor-1, 
                                        Palindrome, 
                                        Max_factor, 
...
is_palindrome(Number) ->
    case (digitize_number(Number) == lists:reverse(digitize_number(Number))) of
        true ->
...
digitize_number(N) when N < 10 ->
    [N];
digitize_number(N) when N >= 10 ->
    digitize_number(N div 10) ++ [N rem 10].

undigitize_number(Number_list) ->
    undigitize_number(Number_list, 0).
undigitize_number([], Number) ->
    Number;
